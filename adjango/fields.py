...
class AManyToManyField(ManyToManyField, Generic[_RM]):
    if TYPE_CHECKING:
        ...
        # descriptor as returning ``AManyRelatedManager[_RM]`` allows static
        # type checkers to properly infer the type of objects returned by
        # methods like ``aall`` or ``all``. Without inheriting from ``Generic``
        # and annotating this method, usages such as ``await
        # order.products.aall()`` would resolve to ``list[_RM]`` instead of
        # ``list[Product]``.
        def __get__(self, instance: 'Model | None', owner: type | None = None) -> AManyRelatedManager[_RM]: ...
    else:  # pragma: no cover - runtime fallback for IDEs when descriptor isn't bound yet
